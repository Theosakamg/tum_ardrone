...
void EstimationNode::Loop()
{
    ...
    while (nh_.ok())
    {
        ...
        pose_stmp.header.frame_id = "/base_frontcam";
        ...
        if (currentKF > lastKF) {
            ...
            point_cloud.header.frame_id = "/base_frontcam";
...
void EstimationNode::dynConfCb(tum_ardrone::StateestimationParamsConfig &config, uint32_t level)
{
    ...
    ptamWrapper->setPTAMPars(
            config.PTAMMinKFTimeDiff,
            config.PTAMMinKFWiggleDist,
            config.PTAMMinKFDist);
